...
from model.faster_rcnn.resnet_dis import resnet as resnet_dis
...
if __name__ == '__main__':
  ...
  if args.net == 'vgg16':
    ...
    fasterRCNN = resnet_dis(imdb.classes, 18, pretrained=True, class_agnostic=args.class_agnostic)
  elif args.net == 'res10':
  ...
  ## setup god model
  fasterRCNNgod = resnet(range(81), 101, pretrained=False, class_agnostic=args.class_agnostic)
  fasterRCNNgod.create_architecture()
  fasterRCNN.create_architecture()
  
  load_name = "/home/ken/caffe2/models/res101/coco/faster_rcnn_1_10_9771.pth"
  print("loading god checkpoint %s" % (load_name))
  checkpoint = torch.load(load_name)
  fasterRCNNgod.load_state_dict(checkpoint['model'])
...
#    lr = optimizer.param_groups[0]['lr']
  ...
  model = nn.Sequential()
  model.add_module('transform', nn.Conv2d(1024,256, kernel_size=1, stride=1, padding=0, bias=False))
  
  checkpoint = torch.load("adaptlayer7.pth")
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      im_data2 = im_data.cpu().numpy()
      for n, std in enumerate(pixel_stdens):
          im_data2[0,n,:,:]*=std
          im_data2[0,n,:,:]+=pixel_means[n]
      im_data2 *= 255. # Convert range to [0,1]         
      im_data2[0,:,:,:]=im_data2[0,::-1,:,:]
      im_data2 = torch.from_numpy(im_data2).cuda()
      
       
      rois, cls_probgod, bbox_predgod, \
      rpn_loss_cls, rpn_loss_box, \
      RCNN_loss_cls, RCNN_loss_bbox, \
      rois_label, basegod  = fasterRCNNgod(im_data2, im_info, gt_boxes, num_boxes)
      
      rois, cls_prob, bbox_pred, \
      rpn_loss_cls, rpn_loss_box, \
      RCNN_loss_cls, RCNN_loss_bbox, \
      rois_label, base  = fasterRCNN(im_data, im_info, gt_boxes, num_boxes)
      
      transbase = model(basegod)
      
      
      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() + ((bbox_predgod[:,:bbox_pred.shape[1],:bbox_pred.shape[2]]-bbox_pred)**2).mean() \
           + ((cls_probgod[:,:cls_prob.shape[1],:cls_prob.shape[2]]-cls_prob)**2).mean() + ((transbase - base)**2).mean()
      ...
      if step % args.disp_interval == 0:
        ...
        if args.mGPUs:
          ...
          loss_bbox = ((bbox_predgod[:,:bbox_pred.shape[1],:bbox_pred.shape[2]]-bbox_pred)**2).mean().item()
          loss_cls = ((cls_probgod[:,:cls_prob.shape[1],:cls_prob.shape[2]]-cls_prob)**2).mean().item()
          ...
          loss_bbox = ((bbox_predgod[:,:bbox_pred.shape[1],:bbox_pred.shape[2]]-bbox_pred)**2).mean().item()
          loss_cls = ((cls_probgod[:,:cls_prob.shape[1],:cls_prob.shape[2]]-cls_prob)**2).mean().item()
          loss_base = ((transbase - base)**2).mean().item()
          fg_cnt = torch.sum(rois_label.data.ne(0))
          bg_cnt = rois_label.data.numel() - fg_cnt
        ...
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, dis_bbox %.4f, dis_cls %.4f, loss_base %.4f" \
                      % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_bbox, loss_cls, loss_base))
        if args.use_tfboard:
          info = {
            'loss': loss_temp,
