...
# do training
command = "python trainval_net_ds_savemod_coco.py --cuda --dataset pascal_voc_"+target+" --net "+args.net+" --r True --s "+SESS+" --checkepoch 40 --checkpoint 625 --checksession 500 --epoch "+str(args.epoch)+"  --bs 1 --nw 8  --lr 1e-4 --lr_decay_step "+str(args.lr_decay_step)
subprocess.call(command, shell=True)
...
for ep in range(4, 13): 
    epoch = str(ep*5)
    command = "python demo-and-eval-save.py --coco True --net "+args.net+" --dataset pascal_voc_"+target+" --cuda --checksession "+SESS+" --checkepoch "+epoch+" --checkpoint "+point+" --image_dir /data2/lost+found/img/"+target+"_val/ --truth output/baseline/"+target+"val-res101.pkl"
    subprocess.call(command, shell=True)
#20
#25
#30
