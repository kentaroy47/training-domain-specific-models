...
#    lr = optimizer.param_groups[0]['lr']
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label, _  = fasterRCNNgod(im_data2, im_info, gt_boxes, num_boxes)
      ...
      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           ...
           + ((cls_probgod[:,:cls_prob.shape[1],:cls_prob.shape[2]]-cls_prob)**2).mean()
      ...
      if step % args.disp_interval == 0:
        ...
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f, dis_bbox %.4f, dis_cls %.4f" \
                      % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_bbox, loss_cls))
        if args.use_tfboard:
          info = {
            'loss': loss_temp,
