...
#  args.set_cfgs = ['ANCHOR_SCALES', '[4, 8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '50'] 
  ...
  if args.net == 'vgg16':
    ...
    if args.coco:
        fasterRCNN = resnet(range(81), 18, pretrained=True, class_agnostic=args.class_agnostic)
    else:
        fasterRCNN = resnet(pascal_classes, 18, pretrained=True, class_agnostic=args.class_agnostic)
...
#      print("roi", rois.shape)
      ...
      if args.save:
          roilist.append(boxes.cpu().numpy())
          base_featlist.append(base_feat.detach().cpu().numpy())
...
#      print("box:",pred_boxes.shape)
      ...
      misc_tic = time.time()
      gputime=time.time()-det_tic
      print("gput",gputime)
      tooktime.append(gputime) 
      im2show = np.copy(im)
...
#            print(a.shape)
      ...
      if args.imshow:
          print("write image", imglist[num_images][:-4])
...
#  if args.coco:
  ...
  for i, clsbox in enumerate(allbox):
      for ii, boxes in enumerate(clsbox):
          ...
          for iii, box in enumerate(boxes):
              out = box[0:4]
              if min(out[2]-out[0],out[3]-out[1]) < 25:
                  dellist.append(iii)
  ...
  if args.save:
      outfile = 'output/' + args.dataset + '-'+ args.net  +'.pkl'
      with open(outfile, 'wb') as f:
          pickle.dump(all_boxes, f, pickle.HIGHEST_PROTOCOL)
      outfile = 'output/bbox-' + args.dataset + '-'+ args.net  +'.pkl'
      with open(outfile, 'wb') as f:
          pickle.dump(roilist, f, pickle.HIGHEST_PROTOCOL)
      outfile = 'output/feat-' + args.dataset + '-'+ args.net  +'.pkl'
      with open(outfile, 'wb') as f:
...
import pickle
outfile = 'time/' + target + '-' + args.net + '.pkl'
with open(outfile, 'wb') as f:
      pickle.dump(tooktime, f, pickle.HIGHEST_PROTOCOL)
